...
class Group
{
	 ...
	 */
	public function setGroup($name, $status, $admin_privilege, $description, $net_type, $net_attribute_type, $net_vlan_id, $ip_limitation_status, $hw_limitation_status, $ip_range_start, $ip_range_stop)
	{
		$this->name = $name;
		$this->status = $status;
	...
	function create()
	{
		...
		$query .= ' VALUES ("' . $this->name . '", ' . $this->status . ', ' . $this->admin_privilege . ', "' . $this->description . '", ' . $this->net_type . ', ' . $this->net_attribute_type . ', ' . $this->net_vlan_id . ', ' . $this->ip_limitation_status . ', ' . $this->hw_limitation_status . ', "' . $this->ip_range_start . '", "' . $this->ip_range_stop . '"' . ')';
	...
	public function changeStatus()
	{
		...
		else
		{
			...
			/* Prepare inserting query */
			$query = "UPDATE net_group " . "SET status ='" . $newStatus . "'" . " WHERE name = '" . $this->name . "'";
	...
	public function addHwLimitedDevice($mac_address)
	{
		...
		else
		{
			...
			$query .= ' VALUES ("' . $this->name . '", "' . $mac_address . '")';
	...
	public function countDisabledGroups()
	{
		...
		$query_result = $this->database->query($query);
		if (!$query_result) return false; //Error
		else return $query_result->fetch_assoc() ["COUNT(name)"];
	}
